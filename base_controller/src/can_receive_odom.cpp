...
class SubscribeAndPublish
{
  ...
  ros::Time current_time, last_time;
	...
	double x, y, vx, vth, th;
  double w_0_, w_1_;
};

SubscribeAndPublish::SubscribeAndPublish():x(0),y(0),vx(0),vth(0),th(0),private_nh("~")
  {

    private_nh.param("w_0", w_0_, 0.5); 
    private_nh.param("w_1", w_1_, 0.5);
...
void SubscribeAndPublish::callback(const sensor_msgs::Imu & cmd_input)
    {
      ...
      geometry_msgs::TransformStamped odom_trans;//创建一个tf发布需要使用的TransformStamped类型消息
       ...
       float covariance[36] = {0.003, 0, 0, 0, 0, 0, //covariance on gps_x
                          0, 0.003, 0, 0, 0, 0, //covariance on gps_y
    ...
    //载入covariance矩阵
       for(int i=0; i<36; i++)
        ...
        //串口接收的数据长度正确就处理并发布里程计数据消息
            ...
            double vth_0 = vel_angular.d * 0.001;

            double delta_th = (w_1_ * vth + w_0_ * vth_0) * dt;
